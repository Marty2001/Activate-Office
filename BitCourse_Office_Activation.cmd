...
cls
color F0
title BitCourse - MS Office Activation  %masver%
...
%eline%
echo Your version [%masver%] is outdated.
echo ________________________________________________
echo:
...
set showfix=1
echo Reboot your machine using the restart option and run the script again.
set fixes=%fixes% %mas%in-place_repair_upgrade
...
echo Checking SvcRestartTask Status          [!taskinfo!]
if not defined showfix (
echo "!taskinfo!" | findstr /i "Removed Not Found" %nul1% && (
set fixes=%fixes% %mas%in-place_repair_upgrade
call :dk_color2 %Blue% "Check this webpage for help - " %_Yellow% " in-place_repair_upgrade"
) || (
echo Reboot your machine using the restart option and run the script again.
)
echo:
